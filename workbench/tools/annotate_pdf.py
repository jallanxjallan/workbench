...
def replace_annotations_with_text(pdf_path: str):
    ...
    input_path = Path(pdf_path).expanduser()
    if not input_path.is_absolute():
        input_path = (Path.cwd() / input_path).resolve()
    upload_dir = Path(os.environ.get("WORKBENCH_UPLOAD_DIR", str(input_path.parent))).expanduser()
    upload_dir.mkdir(parents=True, exist_ok=True)
    output_path = str(upload_dir / f"{input_path.stem}_annotated.pdf")
