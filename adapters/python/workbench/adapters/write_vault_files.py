...
Modes:
- writenew: create-only, target must not exist
- writeback: update-only, target must already exist

Input: NDJSON records on stdin.
Output: NDJSON records on stdout (one output record per input record).
"""
...
def _read_ndjson(stream: Iterable[str]) -> Iterable[dict[str, Any]]:
    for line_no, line in enumerate(stream, start=1):
        ...
        try:
            ...
            raise SystemExit(f"Invalid NDJSON on line {line_no}: {e}")
...
def _get_output_rel_path(rec: dict[str, Any]) -> str:
    ...
    # Deprecated fallback for older pipelines.
    legacy = rec.get("path")
...
def _resolve_under_base(base_dir: Path, rel: str) -> Path:
    ...
    try:
        ...
        raise ValueError(f"Resolved path escapes base_dir: {rel} -> {abs_path}")
    return abs_path
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(description="Write vault files from NDJSON records.")
    p.add_argument("--base-dir", required=True, help="Base directory containing all output")
    p.add_argument(
        "--mode",
        required=True,
    ...
    for rec_no, rec in enumerate(_read_ndjson(sys.stdin), start=1):
        ...
        try:
            ...
            out.update({"error": str(e), "input_record": rec})
            _emit(out)
