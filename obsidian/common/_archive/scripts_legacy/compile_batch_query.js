(async () => {
  ...
  const SELECTION_KEY = "compileBatchSelections_v1";
  const FIELDS = [
  ...
  async function buildSession() {
    ...
    return {
      ...
      gitIndex,
      lastScan: Date.now(),
      sourceKind: tocGroups.length ? "toc" : "filesystem",
  ...
  function scanFilesystem(root, pageCache) {
    ...
    const pages = typeof dv.pages === "function" ? dv.pages(`"${folder}"`) : [];
    const source = typeof pages?.array === "function" ? pages.array() : Array.from(pages || []);

    return source
      .map((page) => buildRow("(All Files)", normalizePath(page?.file?.path || ""), pageCache))
  ...
  function filterRows(sourceSession, activeFilters) {
    ...
    return sourceSession.rows.filter((row) => {
      if (sourceSession.gitIndex.staged.has(row.filePath)) return false;
  ...
  function renderHeading(parent, group, onToggle) {
    ...
    const allSelected = paths.length > 0 && paths.every((path) => selections.has(path));
  ...
  async function render() {
    ...
    containerEl.empty();
    renderHeader(containerEl, session, filteredRows.length, selectedPaths.length, visibleSelected);
    ...
    for (const group of groupedRows) {
      ...
      for (const row of group.rows) {
        ...
        if ((row.class || []).length > 0) {
          const label = item.createSpan({ text: `[${row.class.join(", ")}]` });
  ...
  function renderHeader(parent, sourceSession, visibleCount, selectedCount, visibleSelected) {
    ...
    const sourceLabel = sourceSession.sourceKind === "toc" && sourceSession.sourceFile
      ...
      : `Source: filesystem fallback (${sourceSession.contentRoot})`;
    summary.setText(
      `${sourceLabel} | showing ${visibleCount} of ${sourceSession.rows.length} | selected ${selectedCount} total${visibleSelected !== selectedCount ? `, ${visibleSelected} visible` : ""}`,
    );
  ...
  function createCommandHooks() {
    ...
    return {
      ...
      process: async () => {
        ...
        console.log("Compile Batch Process:", paths);
      ...
      submit: async () => {
        const paths = await getSelectedFilePaths();
        console.log("Compile Batch Submit:", paths);
        if (paths.length > 0 && typeof Notice === "function") {
  ...
  function resolveIndexFile(notePath, fromPath) {
    ...
    if (basename) {
      const matches = app.vault.getMarkdownFiles().filter(
        (file) => normalizeString(file.basename).toLowerCase() === basename.toLowerCase(),
      );
    ...
    return {
      file: null,
      warning: `Could not resolve "${notePath}". Falling back to ${contentRoot}.`,
    };
  }
