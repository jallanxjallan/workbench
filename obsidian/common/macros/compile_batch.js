...
module.exports = async function compileBatch(params = {}) {
  const helper = loadHelper(params);

  try {
    return await helper.runBatchMacro(params, "compile");
  } catch (error) {
    const detail = error?.message || String(error);
    if (typeof Notice === "function") new Notice(detail, 10000);
    console.error(`Compile Batch failed: ${detail}`);
    return null;
  }
};
...
function loadHelper(params = {}) {
  ...
  return require(path.join(String(basePath), "_common", "macros", "_order_safe_batch.js"));
}
