...
def _write_source_file(path: Path, content: bytes = b"source\n") -> Path:
...
def test_compile_assets_handles_file_uri_jpg_updates_frontmatter_and_removes_inline_link(
    ...
    source_path = _write_source_file(tmp_path / "input" / "photo.jpg")
...
def test_compile_assets_handles_file_uri_png(tmp_path: Path) -> None:
    ...
    source_path = _write_source_file(tmp_path / "input" / "diagram.png")
    source_uri = source_path.resolve().as_uri()
...
def test_compile_assets_avoids_duplicate_entries_without_asset_generation(
    tmp_path: Path,
) -> None:
...
def test_compile_assets_does_not_generate_vault_assets_for_file_sources(tmp_path: Path) -> None:
    studio_root, vault_a, assets_a = _create_vault(tmp_path, "vault-a")
    _, vault_b, assets_b = _create_vault(tmp_path, "vault-b")
    source_path = _write_source_file(tmp_path / "input" / "vault_image.jpg")
    source_uri = source_path.resolve().as_uri()
    ...
    assert result.matched_links == 1
    assert updated.metadata["assets"] == []
    assert not any(assets_a.iterdir())
    assert not any(assets_b.iterdir())
...
def test_cli_dispatches_compile_assets_command(tmp_path: Path) -> None:
    ...
    source_uri = _write_source_file(tmp_path / "input" / "photo.jpg").resolve().as_uri()
    markdown = vault_root / "doc.md"
