...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="create-project",
        description=(
            "Attach an Obsidian vault surface to an existing Studio project. "
            "This command does not create Studio projects."
        ),
        epilog=(
            "Studio is the source of truth for project existence. "
            "Vault attachment is optional for headless projects."
        ),
    )
    parser.add_argument(
        "mnemonic",
    ...
    parser.add_argument(
        "--project-root",
        required=True,
        help="Absolute project root where .env.local/.envrc and the vault directory are managed.",
    )
...
def main() -> int:
    ...
    home_dir = Path.home()
    project_root_input = Path(args.project_root).expanduser()
    if not project_root_input.is_absolute():
        die("--project-root must be an absolute path")

    project_root = project_root_input.resolve()
    ...
    if not studio_root_raw:
        die(
            "studio root is not configured. Set AUTOSCRIBE_STUDIO_ROOT "
            "(preferred) or STUDIO_INSTRUCTIONS_ROOT."
        )
    ...
    autoscribe_studio_project = studio_instructions_root / mnemonic
    if not autoscribe_studio_project.exists() or not autoscribe_studio_project.is_dir():
        die(
            "studio project does not exist; refusing vault attachment. "
            f"Expected AUTOSCRIBE_STUDIO_PROJECT at {autoscribe_studio_project}"
        )
    ...
    write_text(
        ...
        (
            ...
            f'AUTOSCRIBE_STUDIO_ROOT="{studio_instructions_root}"\n'
    ...
    print(f"   Studio project (source truth): {autoscribe_studio_project}")
